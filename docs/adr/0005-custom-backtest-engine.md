...
The evolution engine is the second. The survival score, the trial ledger charged at specification time, the held-out vault and the champion/challenger gate are not reporting layered on a backtest; they own when a backtest may run and what a result means (`ARCHITECTURE.md` §10). `BacktestEngine.run()` refuses a `spec_hash` that was never registered (`.claude/rules/overfitting-defences.md`). That is an authority relationship, and inverting it — making our gates a callback inside a foreign `run()` — means the enforcement point moves into code we do not control.
...
- Determinism is testable end to end: the look-ahead probe replays a poisoned future and requires byte-identical decisions before the cut (`.claude/rules/no-lookahead.md`), which needs a loop we control.
