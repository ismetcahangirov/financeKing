...
- Retry a failed window automatically. A fire time is consumed once whatever its outcome, so recovering a failed run is a deliberate backfill. That is the same trade `docs/rules/error-handling.md` makes everywhere else and it is still a real limitation.
