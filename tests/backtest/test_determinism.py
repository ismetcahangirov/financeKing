...
def _run(*, use_run_seed: bool) -> RunTrace:
    ...
    return EventLoop(config, DrawingHandler(use_run_seed=use_run_seed)).run(
        bar_events(START, how_many=BAR_COUNT)
    )
...
def test_the_seeded_draws_are_the_ones_the_run_seed_dictates() -> None:
    ...
    EventLoop(config, first).run(bar_events(START, how_many=BAR_COUNT))
    EventLoop(config, second).run(bar_events(START, how_many=BAR_COUNT))
...
def test_a_different_run_seed_changes_the_trace_and_the_identity_together() -> None:
    ...
    one = EventLoop(config_for(start_utc=START, run_seed=1), DrawingHandler(use_run_seed=True)).run(
        bar_events(START, how_many=BAR_COUNT)
    )
    two = EventLoop(config_for(start_utc=START, run_seed=2), DrawingHandler(use_run_seed=True)).run(
        bar_events(START, how_many=BAR_COUNT)
    )
...
def test_the_trace_digest_changes_when_the_events_change() -> None:
    ...
    config = config_for(start_utc=START)
    shorter = EventLoop(config, RecordingHandler()).run(bar_events(START, how_many=3))
    longer = EventLoop(config, RecordingHandler()).run(bar_events(START, how_many=4))
...
def test_the_digest_is_stable_across_two_traces_built_from_equal_entries() -> None:
    ...
    config = config_for(start_utc=START)
    first = EventLoop(config, RecordingHandler()).run(bar_events(START, how_many=5))
    second = EventLoop(config, RecordingHandler()).run(bar_events(START, how_many=5))
