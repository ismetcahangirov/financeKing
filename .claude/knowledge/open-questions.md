...
- **What the code assumes meanwhile**: conservative configured defaults, with the **quota ledger measuring reality** rather than trusting the configuration ([`../../docs/rules/quota-management.md`](../../docs/rules/quota-management.md)). The ledger is the authority; the configured number is a guess that the ledger corrects. This is the right shape regardless of how OQ-001 resolves, so the work is not wasted — but it is *tuned* wrong until this closes.
