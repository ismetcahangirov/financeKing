...
- **Consequence**: three separate things. First, `resolve_granularity` decides daily-versus-monthly by distance from today and is therefore wrong for both of these datasets, in opposite directions, on every date — so granularity is declared per dataset in `fking.data.alt.registry.ARCHIVE_GRANULARITY` and every fetch passes it explicitly. Second, `ArchiveFormat.epoch_unit` has members for milliseconds and microseconds and none for a datetime string, so `metrics` cannot be expressed in `DECLARED_FORMATS` at all and is deliberately left out of it: it is fetchable and probeable today and not parseable, and `PARSED_SOURCES` records that asymmetry rather than hiding it. Third, this is a fourth instance of the trap class `docs/adr/0013` is about — a timestamp encoding that no field, header or filename announces, where the wrong assumption produces no exception.
- **Re-verify**: on any archive format change, and before implementing the `metrics` parser.
