"""The fill simulator: latency scheduled, fills earned, rejections reported.

This module is the *only* place in the repository where a simulated fill is priced.
`BacktestVenue` and `PaperVenue` are both thin objects around one `FillSimulation`, which
is what makes their agreement structural: there is no second copy of the pricing to drift
from the first. A paper run that filled differently from a backtest of the same bars would
make every backtest result unfalsifiable (`BACKTEST_ENGINE.md` section 3), and the way to
stop that is to have one implementation rather than two disciplined ones.
...
The simulator is mutable, deliberately, and it is infrastructure rather than a domain
object: it holds the working book, the resting queue and the report.
`docs/rules/immutability.md` binds domain types, and every record this class hands out --
`RestingOrder`, `Fill`, `VenueFillRecord`, `VenueReport` -- is frozen.
"""
...
FILL_NAMESPACE: Final[UUID] = uuid5(NAMESPACE_URL, "https://fking.local/backtest/fill")
...
class FillSimulation:
    """The fill model every simulated venue in this repository delegates to.

    Driven in three places and nowhere else: `observe` when a bar closes, `submit` when
    the risk engine has an order, and `resolve_ack` when the ack it scheduled comes back
    around. Everything it knows about the market it learned from `observe`, which is what
    makes look-ahead unavailable rather than merely discouraged.

    It holds no clock. Which instant a decision carries is the caller's to state -- a
    backtest reads it from the event loop, a paper run from wall time -- and a simulator
    that read a clock of its own would be a second source of time in a system whose
    reproducibility rests on there being one.
    """
    ...
    def _print_fill(  # noqa: PLR0913 - every field of the fill it prints; a record here
        ...
        *,
        ...
        # `sim-` rather than `bt-`: the same simulator prints for a backtest and for a
        # paper run, and a prefix naming one of them would be wrong half the time -- in
        # the half that reconciles a paper fill against nothing on the exchange.
        venue_trade_id = f"sim-{order.client_order_id}-{sequence}"
        fill = Fill(
            fill_id=uuid5(FILL_NAMESPACE, venue_trade_id),
